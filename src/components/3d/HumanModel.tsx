...
const ORGAN_MAPPING: Record<string, string[]> = {
  "heart": [
    ...
    "vh_m_heart", 
    "vh_m_heart_valve", "vh_m_mitral_valve", "vh_m_tricuspid_valve", "vh_m_aortic_valve", "vh_m_pulmonary_valve",
    "vh_m_papillary_muscle_of_heart", 
    "vh_m_cardiac_chamber", "vh_m_left_cardiac_atrium", "vh_m_right_cardiac_atrium",
    "vh_m_heart_right_ventricle", "vh_m_heart_left_ventricle",
    "vh_m_interventricular_septum",
    
    "vh_f_heart", 
    "vh_f_heart_valve", "vh_f_mitral_valve", "vh_f_tricuspid_valve", "vh_f_aortic_valve", "vh_f_pulmonary_valve",
    "vh_f_papillary_muscle_of_heart", 
    "vh_f_cardiac_chamber", "vh_f_left_cardiac_atrium", "vh_f_right_cardiac_atrium",
    "vh_f_heart_right_ventricle", "vh_f_heart_left_ventricle",
  ...
  "kidney": [
    ...
    "vh_m_kidney", "vh_m_renal_pelvis", "vh_m_major_calyx", "vh_m_minor_calyx", 
    "vh_m_renal_pyramid", "vh_m_renal_papilla", "vh_m_renal_medulla", 
    "vh_m_cortex_of_kidney", "vh_m_renal_column", "vh_m_ureter",
    
    "vh_f_kidney", "vh_f_renal_pelvis", "vh_f_major_calyx", "vh_f_minor_calyx", 
    "vh_f_renal_pyramid", "vh_f_renal_papilla", "vh_f_renal_medulla", 
    "vh_f_cortex_of_kidney", "vh_f_renal_column", "vh_f_ureter"
  ],
  "intestine": [
    
    "vh_m_intestine", "vh_m_colon", "vh_m_rectal", "vh_m_caecum", 
    "vh_m_ileum", "vh_m_jejunum", "vh_m_duodenum", "vh_m_sigmoid", 
    
    "vh_f_intestine", "vh_f_colon", "vh_f_rectal", "vh_f_caecum", 
    "vh_f_ileum", "vh_f_jejunum", "vh_f_duodenum", "vh_f_sigmoid"
  ],
  "brain": ["brain", "cerebral", "cerebellum", "lobe", "skull", "head"],
  "lungs": [
    
    "vh_m_lungs", "vh_m_hilum", "vh_m_bronchi", "vh_m_bronchus", "vh_m_trachea", "vh_m_larynx",
    "vh_m_bronchopulmonary_segment", "vh_m_lobar_bronchus",
    
    "vh_f_lungs", "vh_f_hilum", "vh_f_bronchi", "vh_f_bronchus", "vh_f_trachea", "vh_f_larynx",
    "vh_f_bronchopulmonary_segment", "vh_f_lobar_bronchus"
  ],
  "pancreas": [
    "vh_m_pancreas", 
    "vh_f_pancreas"
  ...
  "bladder": [
    "vh_m_urinary_bladder", "vh_m_urethra",
    "vh_f_urinary_bladder", "vh_f_urethra"
  ],
  "thyroid": ["vh_m_thyroid", "vh_f_thyroid"],
  "uterus": ["vh_f_uterus"],
  "ovary": ["vh_f_ovary"],
  "eye": [
    "vh_m_eye", "vh_m_ocular", "vh_m_retinal", "vh_m_optic",
    "vh_f_eye", "vh_f_ocular", "vh_f_retinal", "vh_f_optic"
  ],
  "bone": [
    
    "vh_m_bone", "vh_m_femur", "vh_m_tibia", "vh_m_fibula", "vh_m_humerus", 
    "vh_m_radius", "vh_m_ulna", "vh_m_rib", "vh_m_vertebra", "vh_m_skull", 
    "vh_m_pelvis", "vh_m_spine", "vh_m_skeletal", "vh_m_sacrum", "vh_m_coccyx", 
    "vh_m_sternum", "vh_m_scapula", "vh_m_clavicle", "vh_m_patella", 
    "vh_m_ilium", "vh_m_ischium", "vh_m_pubis", "vh_m_hyoid",
    
    "vh_f_bone", "vh_f_femur", "vh_f_tibia", "vh_f_fibula", "vh_f_humerus", 
    "vh_f_radius", "vh_f_ulna", "vh_f_rib", "vh_f_vertebra", "vh_f_skull", 
    "vh_f_pelvis", "vh_f_spine", "vh_f_skeletal", "vh_f_sacrum", "vh_f_coccyx", 
    "vh_f_sternum", "vh_f_scapula", "vh_f_clavicle", "vh_f_patella", 
    "vh_f_ilium", "vh_f_ischium", "vh_f_pubis", "vh_f_hyoid"
  ],
...
const Model = ({ affectedOrgans, modelUrl, gender }: { affectedOrgans: AffectedOrgan[], modelUrl: string, gender: 'male' | 'female' }) => {
  const { scene, nodes } = useGLTF(modelUrl);
  const [tooltip, setTooltip] = useState<{data: any, position: THREE.Vector3} | null>(null);
    ...
    scene.traverse((child) => {
      if (child instanceof THREE.Mesh) {
        ...
        const affected = affectedOrgans.find(a => {
          ...
          if (organKey === "arteries" || organKey === "veins") organKey = "blood";
        ...
        if (affected) {
          ...
          console.log(`Highlighting ${nodeName} for ${affected.organ} (${affected.risk})`);

          
          child.raycast = child.userData.originalRaycast;
        } else {
          
          if (nodeName.includes("bone") || nodeName.includes("vertebra") || nodeName.includes("rib") || nodeName.includes("skull") || nodeName.includes("pelvis") || nodeName.includes("sternum") || nodeName.includes("scapula") || nodeName.includes("clavicle")) {
  ...
  return (
    <>
      <Stage environment={theme === 'dark' ? "city" : "studio"} intensity={0.5} adjustCamera={false} shadows={false}>
        <primitive 
          object={scene} 
          onPointerOver={(e: any) => {
            e.stopPropagation();
      ...
      {tooltip && (
        <Html position={tooltip.position} style={{ pointerEvents: 'none', zIndex: 100 }}>
          <div className={`
          ...
          `}>
            <h4 className="font-bold text-[10px] uppercase tracking-wider mb-1">{tooltip.data.organName}</h4>
            <div className="text-[9px] font-bold mb-1 opacity-90">Risk: {tooltip.data.risk}</div>
            <p className="text-[9px] opacity-80 leading-tight">{tooltip.data.description}</p>
          </div>
        </Html>
      )}
    </>
      ...
      {hasAnalyzed && affectedOrgans.length === 0 && (
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20 pointer-events-none w-full px-4">
          <div className="bg-green-500/10 backdrop-blur-sm border border-green-500/30 px-6 py-4 rounded-xl text-center shadow-[0_0_30px_rgba(34,197,94,0.2)]">
            ...
            <p className="text-green-200/70 text-sm">This food is healthy for your body!</p>
          </div>
        </div>
      )}
